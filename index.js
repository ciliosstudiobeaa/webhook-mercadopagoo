...
app.post("/gerar-pagamento", async (req, res) => {
  try {
    ...
    if (!nome || !whatsapp || !servico || precoTotal == null || !diaagendado || !horaagendada) {
    ...
    const whatsapp_prefill = buildWhatsAppLink({ nome, servico, diaBr, hora: horaagendada, extra: "%0A%0A(Enviei pelo site)" });
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    if (payment.status === "approved") {
      ...
      try { await postToGoogleScript(agendamento, 3); } catch (err) { console.error(err.message); }

      agendamentos.push({ data: diaIso, horario: hora, servico, nome });

      const waLink = buildWhatsAppLink({ nome, servico, diaBr: isoToBR(diaIso), hora });
